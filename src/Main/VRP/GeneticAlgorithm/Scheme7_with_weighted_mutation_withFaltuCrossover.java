...
import Main.VRP.Individual.Crossover.FaltuSystemCrossOver;
...
public class Scheme7_with_weighted_mutation_withFaltuCrossover implements GeneticAlgorithm
{
	...
	public Scheme7_with_weighted_mutation_withFaltuCrossover(ProblemInstance problemInstance) 
	{
	...
	public Individual run() 
	{
		...
		/////////////////////////////////////////////
		int unimproved=0;
		double currentValue=population[0].costWithPenalty;
		///////////////////////////////////////////////
		...
		for( generation=0;generation<NUMBER_OF_GENERATION;generation++)
		{
			if(Solver.BKSValueSet){
				if(currentValue==population[0].costWithPenalty) unimproved++;
				else {
					currentValue=population[0].costWithPenalty;
					unimproved=0;
				}
				if(unimproved>20 && currentValue<=Solver.BKSValue){
					break;
				}
			}
			
			
			System.out.println(generation+", "+population[0].cost+", "+population[0].costWithPenalty+", "+population[0].isFeasible);
			...
			FaltuSystemCrossOver.crossOver(problemInstance, parent1, parent2, offspring1, offspring2);
